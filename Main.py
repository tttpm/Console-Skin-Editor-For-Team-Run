...
print_logo()
version = '0.1'
print(default+'Console Skin Editor for Team Run v'+version+' by '+color(255,102,0)+'Tipim'+default)
#yes, i again took code from TRIC :)
try:
            ...
            if action == 'cl': picture = [[a] * 20 for i in range (18)][::]
                ...
                if amogum == 't':
                    ...
                    if inp.lower() in ['suncat', 'tdf', 'солнцекот', 'cjkywtrjn']:
                        picture = copy(import_skin(suncat))
                    else:
                        picture = copy(import_skin(inp))
                    print('press [esc] to continue')
                    kb.wait("esc")
                    continue
                elif amogum == 'i':
...
(default - trSkin1MzCAAGuDUcYAMRwNQNDNDcFwgwJ6MIaQU8lz/Ah0s6OriamlCzKDQsWjbh518wC6ebTcINLNZi5GTmZGQMVwBmYM0pMx6uZRN1Pi5kHieOo3+Sizi55OHSRuBgA=)
''')
